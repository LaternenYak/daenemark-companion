    ...
    if eingabe:
        try:
            if richtung == "Dänisch ➔ Deutsch":
                ergebnis = GoogleTranslator(source='da', target='de').translate(eingabe)
            else:
                ergebnis = GoogleTranslator(source='de', target='da').translate(eingabe)
                
            st.success(f"**Übersetzung:** {ergebnis}")
            
        except Exception as e:
            st.error(f"Fehler bei der Übersetzung: {e}")import streamlit as st
from deep_translator import GoogleTranslator
